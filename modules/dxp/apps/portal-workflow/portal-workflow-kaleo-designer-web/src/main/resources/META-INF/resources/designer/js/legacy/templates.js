...
AUI.add(
	...
	(A) => {
		...
		const TPL_SELECT_MULTIPLE = [
			...
			'</tpl>',
			'<select class="{[A.TplSnippets.getClassName(values.auiCssClass, values.cssClass)]}" <tpl if="values.disabled">disabled="disabled"</tpl> <tpl if="values.multiple">multiple="multiple"</tpl> id="{id}" name="{name}">',
			'<tpl for="options">',
